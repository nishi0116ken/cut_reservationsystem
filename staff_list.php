...
<body>
    ...
    try{
        ...
        $sql = 'SELECT id,name FROM b_staff WHERE 1';
        ...
        echo '<h3 class="tuika">スタッフ管理画面</h3>';

        echo '<form class="tuika list" method="post" action ="staff_branch.php">';
        while(true){
            $rec = $stmt->fetch(PDO::FETCH_ASSOC);
            if($rec==false){
                break;
            }
            echo '<input class="tuika" type="radio" name="staffcode" value="'.$rec['id'].'">';
            echo $rec['name'];
            echo '<br />';
        }
